...
void 
ctFileReader_openFile(char* filename, RabbitCtFile* ctFile)
{
    ...
    if (ctFile->file != NULL)
    {
        if (!fread((void*) &ctFile->header, sizeof(RabbitCtHeader),
                    1, ctFile->file))
        {
    ...
    printf("Opened CT Data file: %d projections with size %d X %d\n",
            ctFile->header.numberOfImages,ctFile->header.imageDimension[0],ctFile->header.imageDimension[1]);
}

void 
ctFileReader_readGeometry(RabbitCtFile* ctFile, double* projectionData )
{
    if (!fread((void*) projectionData,12*sizeof(double),
                ctFile->header.numberOfImages, ctFile->file))
    {
...
int 
ctFileReader_readImage(RabbitCtFile* ctFile, double* matrix, float* image)
{
        currentImage++;
...
void 
ctFileReader_close(RabbitCtFile* ctFile)
{
    fclose(ctFile->file);
}
