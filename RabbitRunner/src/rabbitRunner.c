...
int main(int argc, char **argv) {
   ...
   * *****************************************************/
  {
    ...
    if (opt_verbose) {
      printf("Processing %d projections with size %d X %d\n",
             globalNumberOfProjections, data.imageWidth, data.imageHeight);
    ...
    data.projectionBuffer =
        (Projection *)malloc(bufferSize * sizeof(Projection));

    for (int i = 0; i < bufferSize; i++) {
      data.projectionBuffer[i].matrix = (double *)malloc(12 * sizeof(double));
      memoryUtils_allocate(&data.projectionBuffer[i].image,
    ...
    while (counter > 0) {
      ...
      for (int i = 0; i < bufferSize; i++) {
        data.projectionBuffer[i].id =
            ctFileReader_readImage(&inputFile, data.projectionBuffer[i].matrix,
  ...
  if (refFilename != NULL) {
    ...
    if (file != NULL) {
      if (!fread((void *)referenceVolume, sizeof(uint16_t), numberOfVoxel,
                 file)) {
    ...
    fwrite((rec_volume +
            (data.problemSize / 2 - 1) * data.problemSize * data.problemSize),
  ...
  double time_mean =
      (double)time / (double)(1000.0 * (double)globalNumberOfProjections);
  printf("\n--------------------------------------------------------------\n");
